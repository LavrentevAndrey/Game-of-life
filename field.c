...
char neighbor_count_simple(char **prev_gen, int y, int x) {
    ...
    return prev_gen[y][x] == 1 ? count - 1: count; // Вычитаем единицу если посчитали искомую клетку
}
...
char neighbor_count_hard(Field *field, int y, int x) {
    ...
    for (int i = y - 1; i < y + 2; i++) {
        for (int j = x - 1; j < x + 2; j++) {
            count += field->gen[(i + max_y) % max_y][(j + max_x) % max_x]; // Считаем сумму всех значений соседей
        }
    }

    return field->gen[y][x] == 1 ? count - 1: count; // Вычитаем единицу если посчитали искомую клетку
}
